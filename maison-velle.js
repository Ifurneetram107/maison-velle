/* ============================================================
   maison-velle.js
   Paste your Supabase credentials below, then include this
   file on every page AFTER the Supabase CDN script.
   ============================================================ */
...
function mvBuildProdCard(p, delay) {
  ...
  el.innerHTML = `
    <div class="prod-img" style="background-image:url('${p.image_url || ''}');background-size:cover;background-position:center">
...
function mvBuildNiCard(p) {
  ...
  el.innerHTML = `
    <div class="ni-thumb" style="background-image:url('${p.image_url || ''}');background-size:cover;background-position:center">
...
async function mvLoadBestSellers() {
  ...
  if (error || !data?.length) return; // keep fallback if error

  grid.innerHTML = '';
  data.forEach((p, i) => grid.appendChild(mvBuildProdCard(p, i + 1)));

  // Re-init QuickView + scroll reveal for dynamic cards
  mvInitQuickView();
  mvInitReveal();
}
